#ifndef CEdgeDetection
#define CEdgeDEtection
...
class Matt{
    ...
    static Matt<int> sobelGradient(Matt<T>&, Matt<int>&);
    static Matt<T> gradientMagnitude(Matt<int>&, Matt<int>&);
    static Matt<T> sobelEdgeDetector(Matt<T>&);
    static Matt<int> gradientPhase(Matt<int>& gX, Matt<int>& gY);
    static void nonMaximalFilter(Matt<T>& mag, Matt<int>& ph);
    static void cannyEdgeDetector(Matt<T>& gimage);
    static void histThreshold(Matt<T>& edgeMap, T min, T max);
